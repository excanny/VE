...
class Chat{
    ...
    private $user  = 'verityge_verityg';
    private $password   = "SGA#?^X8E%Oq";
    private $database  = "verityge_ve";      
	...
	public function loginUsers($username, $password){
		...
		$sqlQuery = "
		SELECT *
		 FROM ".$this->chatUsersTable." WHERE email='".$username."'";
		 $row = $this->getData($sqlQuery);
		 //var_dump($row[0]['pass_word']);
		 if (password_verify($password, $row[0]['pass_word'])) 
		 {			
	...
	public function getUserChat($from_user_id, $to_user_id) {
		...
		foreach($userChat as $chat){
			...
			if($chat["sender_userid"] == $from_user_id) {
				...
				$conversation .= '<img width="22px" height="22px" src="http://veritygeo.com/demo/dashboard/uploads/images/profile-photos/100/'.$fromUserAvatar.'" alt="" />';
			} else {
				$conversation .= '<li class="replies">';
				$conversation .= '<img width="22px" height="22px" src="http://veritygeo.com/demo/dashboard/uploads/images/profile-photos/100/'.$toUserAvatar.'" alt="" />';
			}			
	...
	public function showUserChat($from_user_id, $to_user_id) {		
		...
		foreach ($userDetails as $user) {
			$toUserAvatar = $user['photo'];
			$userSection = '<img src="userpics/'.$user['photo'].'" alt="" />
				<p>'.$user['email'].'</p>
